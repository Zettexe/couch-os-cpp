...
void roll::error(const wiz::string &dice_string, const wiz::string &error_message, const int error_position, const int error_position_length) {
...
int apply_operator(int op1, int op2, char op, roll_flags_d flags, roll_result_d *result) {
  switch (op) {
    ...
    case 'd':
      ...
      for (size_t i = 0; i < result->rolls.size(); i++) {
        result->rolls[i].erase(std::remove(result->rolls[i].begin(), result->rolls[i].end(), 0), result->rolls[i].end());
...
roll_flags_d roll::parse_modifiers(std::string modifiers) {
  ...
  while (it != end) {
    ...
    if (!reroll_operator.empty()) {
      ...
      if (flags.reroll.first > 10000 || (flags.reroll.second != std::numeric_limits<int>::max() && flags.reroll.second > 10000)) {
        error(modifiers, "Number has to be less than 10k!");
        flags.is_error = true;
        return flags;
      } else if ((flags.reroll.first != -std::numeric_limits<int>::max() && flags.reroll.first < 0) || flags.reroll.second < 0) {
        error(modifiers, "Number has to be greater than 0!");
        flags.is_error = true;
        return flags;
      }
...
void roll::command_definition() {
  ...
  output << event->command.usr.get_mention() << " rolled `[" << input_string << "]`: `" << data.total << "`\n```js" << std::endl;
  ...
  for (roll_result_d result : data.results) {
    ...
    for (size_t i = 0; i < result.rolls.size(); i++) {
      ...
      for (size_t j = 0; j < result.rolls[i].size(); j++) {
        ...
        if (std::find(result.reroll_positions.begin(), result.reroll_positions.end(), std::make_pair(i, j)) != result.reroll_positions.end()) {
