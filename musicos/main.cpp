...
void initialize_logging() {
  ...
  if (count > 0) {
    sinks.push_back(std::make_shared<spdlog::sinks::daily_file_format_sink_st>("logs/%Y-%m-%d." + std::to_string(count) + ".log", 0, 0));
  } else {
    sinks.push_back(std::make_shared<spdlog::sinks::daily_file_format_sink_st>("logs/%Y-%m-%d.log", 0, 0));
  }
...
int main() {
  ...
  bot.on_guild_create([&bot](dpp::guild_create_t event) {
    bot.guild_bulk_command_create(commands_vector, event.created->id, [event](dpp::confirmation_callback_t callback) {
      ...
      if (callback.is_error()) {
        ...
        for (const auto &error_detail : error.errors) {
          spdlog::error("DETAIL ({}): Field: {}, Object: {}, Reason: {}", error_detail.code, error_detail.field, error_detail.object, error_detail.reason);
        }
        return;
      }
